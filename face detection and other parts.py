...
#classifier3 = cv2.CascadeClassifier("haarcascade_lowerbody.xml")
...
while True:
    ...
    if ret:
        ...
        #lower_bodys = classifier3.detectMultiScale(frame)
        ...
        for l in lower:
            x3,y3,w3,h3 = l
            cv2.rectangle(l,(x3,y3),(x3+w3,y3+h3),(0,255,255),6)
            roi_gray = gray[y3:y3+h3, x3:x3+w3]
            roi_color = frame[y3:y3+h3, x3:x3+w3]
            #x2,y2,w2,h2 = lower_body
            #frame = cv2.rectangle(frame,(x2,y2),(x2+w2,y2+h2),(0,255,255),4)
